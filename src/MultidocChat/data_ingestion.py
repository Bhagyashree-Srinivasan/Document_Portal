        ...
        try:
            ...
            for uploaded_file in uploaded_files:
                ...
                if ext not in self.SUPPORTED_FILE_TYPES:
                    self.log.warning("", filename = uploaded_file.name)
                    continue
                unique_filenname = f"{uuid.uuid4().hex[:8]}{ext}"
                temp_path = self.session_temp_dir / unique_filenname

                with open(temp_path, 'wb') as f:
                    f.write(uploaded_file.read())
                self.log.info("File saved for Ingestion", filename = uploaded_file.name, 
                              temp_path = str(temp_path), session_id = self.session_id)
            ...
            self.log.error(f"Error during file ingestion:", error = {str(e)})
